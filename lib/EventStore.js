...
EventStore.prototype = {
	...
	storeEvent : function(aggreId,aggreState,event,callback){
		...
		self._driver.eventCount(aggreId,function(err,count){

		self._driver.getSnapshot(aggreId,function(err,snapshot){


				event._data.num = count;
				...
				if(!snapshot || snapshot.events.length >= self._create_snapshot_freq){
					...
					ss._data.aggreState = aggreState;
	...
	findEvents : function(aggreId,start,limit,callback){
		this._driver.findEvents2.apply(this._driver,arguments);
	},

	count:function(aggreId,callback){
		this._driver.count.apply(this._driver,arguments)
	}
