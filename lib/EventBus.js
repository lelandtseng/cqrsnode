...
publish:function publish(event){
	...
	if(!event.aggreType){
		...
		this.emitter.emit(event.aggreType+event.aggreId+event.name,domain);	
		this.emitter.emit(event.aggreType+event.name,domain);
		this.emitter.emit(event.aggreType,event,domain);
	}
},
