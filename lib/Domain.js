...
function Domain(cfg,name){
	...
	if(arguments.length === 1 && typeof arguments[0] === 'string'){
		if(Domain.get(arguments[0])) throw new Error('have same name lib. please update name.');
		else{
			name = arguments[0]	
		}
	...
	{
			if(Domain.get(arguments[1])) throw new Error('have same name lib. please update name.');
			else{
				name = arguments[1]	
			}
	}else 
	...
	var serviceBus = new ServiceBus(loader.Aggres,loader.services);
	var commandBus = new CommandBus(loader.Aggres,serviceBus);
	var eventBus = new EventBus(loader.Aggres,serviceBus);
	var door = new Door(queryBus,commandBus,eventBus,loader.DB);
