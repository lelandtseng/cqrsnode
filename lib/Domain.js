...
function Domain(cfg,name){
	...
	if(arguments.length === 1 && typeof arguments[0] === 'string'){
		if(Domain.get(arguments[0])) throw new Error('have same name appdomain. please update name.');
		else{
			name = arguments[0]	
		}
	...
	{
			if(Domain.get(arguments[1])) throw new Error('have same name appdomain. please update name.');
			else{
				name = arguments[1]	
			}
	}else 
